...
# Load data
u = Utils()
#train_facile = u.load_matrix('data/data_train_difficile.mat')

#generate pairs
#pairs_idx, pairs_label = u.generate_pairs(train_facile['label'], 1000, 0.1)
#newX,newY = u.select_pairs_data(pairs_idx,train_facile['X'],train_facile['label'],c=800)
#feat_idx = u._feat_idx

#test gradient
#g = Gradient()
#M_ini = g.generate_I(newX.shape[1])
#M = g.sgd_metric_learning(newX, newY, 0.002, 20000, 0, M_ini)
...
#X /= X.std(axis=0)
#X[np.isnan(X)] = 0.
#dist = m.braycurtis_pond_dist(X, pairs_idx,M)
...
test_facile = u.load_matrix('data/data_test_difficile.mat')
